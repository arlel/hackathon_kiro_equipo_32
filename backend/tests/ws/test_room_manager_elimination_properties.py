...
def test_property_14_turn_increment(initial_turns: int) -> None:
    ...
    room = Room(code="ABC123", config=RoomConfig(format="commander", starting_life=40, turn_counter_enabled=True))
...
def test_property_18_last_player_standing(num_dead: int, survivor_life: int) -> None:
    ...
    survivor = PlayerState(id="survivor", username="Survivor", life=survivor_life, websocket=None)
    room.players["survivor"] = survivor
    for i in range(num_dead):
        dead = PlayerState(id=f"dead{i}", username=f"Dead{i}", life=-(i + 1), websocket=None)
...
def test_property_19_sequential_elimination_order(num_eliminations: int) -> None:
    ...
    orders = sorted(p.elimination_order for p in room.players.values() if p.elimination_order is not None)
...
def test_property_19_sequential_after_revive(num_players: int, revive_index: int) -> None:
