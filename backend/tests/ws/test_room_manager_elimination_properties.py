...
def test_property_14_turn_increment(initial_turns: int) -> None:
    ...
    room = Room(
        code="ABC123",
        config=RoomConfig(format="commander", starting_life=40, turn_counter_enabled=True),
    )
...
def test_property_19_sequential_elimination_order(num_eliminations: int) -> None:
    ...
    orders = sorted(
        p.elimination_order for p in room.players.values() if p.elimination_order is not None
    )
